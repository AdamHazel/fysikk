...
k = 200.0  # N/m
L0 = 5.0  # m
omega = 10.0  # s^-1 (vinkelhastighet)
radius = 10.0  # m (sirkelradius)

# Startbetingelser i henhold til oppgaveteksten
x1_init = 0.0     # (x1, y1) = (0 m, 10 m)
y1_init = radius
vx1_init = 0.0     # (v1x, v1y) = (0 m/s, 0 m/s)
vy1_init = 0.0

x2_init = 10.0     # (x2, y2) = (10 m, -10 m)
y2_init = -10.0
vx2_init = 10.0    # (v2x, v2y) = (10 m/s, 20 m/s)
vy2_init = 20.0

initial_state = [
    x1_init, y1_init,  # x1, y1
    vx1_init, vy1_init, # vx1, vy1
    x2_init, y2_init,   # x2, y2
    vx2_init, vy2_init   # vx2, vy2
]
...
def system_equations(state, t):
    """
    Differensialligninger for to-partikkel fjærsystem med tvungen sirkelbevegelse for partikkel 1
    """
    x1, y1, vx1, vy1, x2, y2, vx2, vy2 = state

    # Beregn ny posisjon for partikkel 1 (tvungen sirkelbevegelse)
    angle = omega * t
    x1_new = radius * np.sin(angle)
    y1_new = radius * np.cos(angle)

    # Beregn nye hastigheter for partikkel 1
    vx1_new = radius * omega * np.cos(angle)
    vy1_new = -radius * omega * np.sin(angle)

    # Beregn fjærkraft mellom partiklene
    dx = x2 - x1
    dy = y2 - y1
    current_length = np.sqrt(dx ** 2 + dy ** 2)

    if current_length == 0:
        return [vx1_new, vy1_new, 0, 0, vx2, vy2, 0, 0]
    ...
    # Returner deriverte verdier
    return [vx1_new, vy1_new, 0, 0, vx2, vy2, ax2, ay2]
...
pygame.display.set_caption("To-Partikkel Fjærsystem - Sirkelbevegelse")
...
def draw_circle_path():
    """Tegner sirkelbanen for partikkel 1"""
    pygame.draw.circle(screen, (100, 100, 100), to_screen(0, 0), int(radius * scaling_factor), 1)

def draw_particles(x1, y1, x2, y2):
    pygame.draw.circle(screen, (255, 0, 0), to_screen(x1, y1), 10)  # Partikkel 1
    pygame.draw.circle(screen, (0, 0, 255), to_screen(x2, y2), 10)  # Partikkel 2
...
def draw_spring(x1, y1, x2, y2):
    ...
    # Tegn fjæren som en enkel linje
